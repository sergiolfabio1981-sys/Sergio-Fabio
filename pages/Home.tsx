        ...
        {heroSlides.map((slide, index) => (
            ...
            >
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 pt-10">
                    <h2 className={`text-5xl md:text-7xl font-black text-white mb-4 drop-shadow-2xl animate-fade-in-up tracking-tight uppercase`}>{slide.title}</h2>
                    <p className="text-xl md:text-2xl text-gray-200 mb-8 max-w-3xl font-light drop-shadow-lg animate-fade-in-up" style={{animationDelay: '0.2s'}}>{slide.subtitle}</p>
                    <Link to={slide.ctaLink} className="px-8 py-4 rounded-full font-bold text-lg text-white shadow-2xl transition-transform transform hover:scale-105 hover:shadow-orange-500/50 animate-fade-in-up bg-gradient-to-r from-orange-500 to-red-600 border border-white/20" style={{animationDelay: '0.4s'}}>{slide.ctaText}</Link>
                </div>
            </div>
        ))}
        {/* Controls */}
        <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 z-20 p-3 bg-white/10 hover:bg-white/20 text-white rounded-full backdrop-blur-sm hidden md:block"><svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg></button>
        <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 z-20 p-3 bg-white/10 hover:bg-white/20 text-white rounded-full backdrop-blur-sm hidden md:block"><svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></button>
        ...
        <div className="absolute bottom-10 left-0 right-0 z-30 px-4">
             <div className="bg-white/95 backdrop-blur-md p-2 rounded-full shadow-2xl flex flex-col md:flex-row items-center max-w-3xl mx-auto border border-white/50">
                <div className="flex-grow w-full md:w-auto relative">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><svg className="h-6 w-6 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>
                  <input type="text" placeholder="Busca por destino, hotel o experiencia..." className="w-full pl-12 pr-4 py-3 rounded-full text-gray-800 focus:outline-none bg-transparent text-lg placeholder-gray-500" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
                <button className="w-full md:w-auto mt-2 md:mt-0 bg-cyan-700 hover:bg-cyan-800 text-white font-bold py-3 px-8 rounded-full transition-all duration-200 shadow-md" onClick={() => { const section = document.getElementById('explore-section'); if(section) section.scrollIntoView({ behavior: 'smooth' }); }}>Buscar</button>
              </div>
        </div>
      </div>
        ...
        {combinedOffers.length > 0 && (
          <div className="mb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {combinedOffers.slice(0, 6).map((item, idx) => (
        ...
        <div className="mt-8" id="explore-section">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 border-b border-gray-200 pb-4 gap-4">
             <div>
                ...
                <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mt-1">Todas las Opciones</h2>
             </div>
             
             <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide w-full md:w-auto">
                 {categories.map(cat => (
                     <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all ${activeCategory === cat.id ? 'bg-cyan-600 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200'}`}>
                         {cat.label}
                     </button>
                 ))}
             </div>
          ...
          {filteredItems.length === 0 ? (
             <div className="text-center py-20 bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <button onClick={() => { setSearchTerm(''); setActiveCategory('all'); }} className="mt-4 text-cyan-600 hover:text-cyan-800 underline font-medium">Ver todo el catálogo</button>
             </div>
          ) : (
