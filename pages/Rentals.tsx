...
const Rentals: React.FC = () => {
  ...
  useEffect(() => {
    const data = getRentals().map(r => ({...r, type: 'rental' as const}));
    setRentals(data);
    
    // Extract unique amenities for filter list
    const amenitiesSet = new Set<string>();
    data.forEach(r => r.amenities.forEach(a => amenitiesSet.add(a)));
  ...
  useEffect(() => {
    ...
    // 2. Price Range (Convert stored price to current currency for comparison)
    if (minPrice !== '') {
      result = result.filter(r => convertPrice(r.pricePerNight) >= Number(minPrice));
    }
    if (maxPrice !== '') {
      result = result.filter(r => convertPrice(r.pricePerNight) <= Number(maxPrice));
    }
  ...
  }, [rentals, searchTerm, minPrice, maxPrice, minBedrooms, minGuests, selectedAmenities, sortOrder, currency]); // Add currency dependency
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <button 
            className="lg:hidden w-full bg-white p-3 rounded-lg shadow text-gray-700 font-bold flex justify-between items-center"
        ...
        <aside className={`lg:w-1/4 space-y-6 ${isSidebarOpen ? 'block' : 'hidden lg:block'}`}>
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                ...
                <input 
                    ...
                    className="w-full border border-gray-300 rounded-lg p-2 focus:ring-cyan-500 outline-none text-sm"
                    value={searchTerm}
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                ...
                <div className="space-y-3">
                    ...
                    <div>
                        ...
                        >
                            {[1, 2, 3, 4, 5, 6, 8, 10].map(num => <option key={num} value={num}>{num}+</option>)}
                        </select>
                    </div>
                </div>
            </div>
        ...
        <div className="lg:w-3/4 flex flex-col">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-4">
                    {/* Sort */}
                    <select 
                        className="border border-gray-300 rounded-lg p-2 text-sm bg-white focus:ring-cyan-500 outline-none"
                        value={sortOrder}
                        onChange={(e) => setSortOrder(e.target.value as any)}
                    >
                        <option value="default">{t('filters.featured')}</option>
                        <option value="asc">{t('filters.priceLow')}</option>
                        <option value="desc">{t('filters.priceHigh')}</option>
                    </select>
                    ...
                    <div className="bg-gray-100 p-1 rounded-lg flex">
                        <button 
                            ...
                            className={`p-2 rounded-md transition-colors ${viewMode === 'list' ? 'bg-white shadow text-cyan-600' : 'text-gray-500 hover:text-gray-700'}`}
                            title="Vista Lista"
                        >
                        ...
                        <button 
                            ...
                            className={`p-2 rounded-md transition-colors ${viewMode === 'map' ? 'bg-white shadow text-cyan-600' : 'text-gray-500 hover:text-gray-700'}`}
                            title="Vista Mapa"
                        >
            ...
            {viewMode === 'list' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {filteredRentals.map(rental => (
                        <TripCard key={rental.id} trip={rental} />
                    ))}
                ...
                <div className="h-[600px] w-full rounded-xl overflow-hidden shadow-lg border border-gray-200 z-0">
                    <MapContainer 
                        center={[-27.5954, -48.5480]} // Default Floripa
                        zoom={5} 
                    ...
                    >
                        ...
                        />
                        {filteredRentals.map(rental => rental.lat && rental.lng && (
                            <Marker key={rental.id} position={[rental.lat, rental.lng]}>
                                <Popup>
                                    <div className="w-48">
                                        <img src={rental.images[0]} alt={rental.title} className="w-full h-24 object-cover rounded mb-2" />
                                        <h3 className="font-bold text-sm mb-1">{rental.title}</h3>
                                        <p className="text-cyan-600 font-bold">{formatPrice(rental.pricePerNight)}</p>
                                        <Link to={`/rentals/${rental.id}`} className="text-xs underline text-blue-500 block mt-1">Ver propiedad</Link>
                                    </div>
                                </Popup>
                            </Marker>
                        ))}
