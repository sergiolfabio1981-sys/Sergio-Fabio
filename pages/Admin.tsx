                  ...
                  <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6">
                      ...
                      {editingInstallment && (
                          <form onSubmit={handleSave} className="space-y-4">
                              ...
                              <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold">Título</label><input className="w-full border p-2 rounded" value={editingInstallment.title} onChange={e=>setEditingInstallment({...editingInstallment, title: e.target.value})} required /></div><div><label className="block text-sm font-bold">Ubicación</label><input className="w-full border p-2 rounded" value={editingInstallment.location} onChange={e=>setEditingInstallment({...editingInstallment, location: e.target.value})} required /></div></div>
                              <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold">Precio Total</label><input type="number" className="w-full border p-2 rounded" value={editingInstallment.totalPrice} onChange={e=>setEditingInstallment({...editingInstallment, totalPrice: parseFloat(e.target.value)})} required /></div><div><label className="block text-sm font-bold">Fecha Salida (YYYY-MM-DD)</label><input type="date" className="w-full border p-2 rounded" value={editingInstallment.departureDate} onChange={e=>setEditingInstallment({...editingInstallment, departureDate: e.target.value})} /></div></div>
