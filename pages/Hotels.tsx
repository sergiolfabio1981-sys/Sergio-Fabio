...
const Hotels: React.FC = () => {
  ...
  useEffect(() => {
    const data = getHotels().map(h => ({...h, type: 'hotel' as const}));
    setHotels(data);
    
    const amenitiesSet = new Set<string>();
    data.forEach(h => h.amenities.forEach(a => amenitiesSet.add(a)));
                ...
                <div className="flex gap-1">
                    {[1,2,3,4,5].map(star => (
                        <button 
                            ...
                            className={`flex-1 py-2 rounded border text-sm font-bold transition-colors ${minStars === star ? 'bg-yellow-400 text-white border-yellow-500' : 'bg-white text-gray-500 hover:bg-gray-50'}`}
                        >
                            {star}★
                        </button>
                    ))}
                ...
                <div className="space-y-2">
                    {allAmenities.map(amenity => (
                        <label key={amenity} className="flex items-center space-x-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" className="rounded text-blue-600 focus:ring-blue-500" checked={selectedAmenities.includes(amenity)} onChange={() => toggleAmenity(amenity)} />
        ...
        <div className="lg:w-3/4 flex flex-col">
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-4 w-full sm:w-auto">
                    <select className="border rounded-lg p-2 text-sm bg-white focus:ring-blue-500 outline-none flex-grow sm:flex-grow-0" value={sortOrder} onChange={(e) => setSortOrder(e.target.value as any)}>
            ...
            {viewMode === 'list' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {filteredHotels.map(hotel => (
                        <TripCard key={hotel.id} trip={hotel} />
                    ))}
                    {filteredHotels.length === 0 && (
                        <div className="col-span-full text-center py-10">
                            <p className="text-gray-500 text-lg">No se encontraron hoteles con esos filtros.</p>
                            <button 
                                onClick={() => {
                                    setSearchTerm('');
                                    setMinPrice('');
                    ...
                    <MapContainer center={[-27.5954, -48.5480]} zoom={11} scrollWheelZoom={true} style={{ height: "100%", width: "100%" }}>
                        ...
                        {filteredHotels.map(h => h.lat && h.lng && (
                            <Marker key={h.id} position={[h.lat, h.lng]}>
                                <Popup>
                                    <div className="w-48">
                                        <img src={h.images[0]} className="w-full h-24 object-cover rounded mb-2" />
                                        <h3 className="font-bold text-sm mb-1">{h.title}</h3>
                                        <p className="text-blue-600 font-bold">{formatPrice(h.pricePerNight)}</p>
                                        <Link to={`/hotels/${h.id}`} className="text-xs underline text-blue-500 block mt-1">Ver Hotel</Link>
                                    </div>
                                </Popup>
                            </Marker>
                        ))}
